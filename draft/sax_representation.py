...
def sax(series, len_window, dim_window, alphabet_size, stride):
    ...
    normalized_series = scaler.fit_transform(
        series.reshape(-1, 1)
    ).flatten()  # Normalisation par ligne

    # 2. on découpe la en fenêtres de taille 'len_window' : sliding window
    n = len(normalized_series)
    windows = [
        normalized_series[i : i + len_window]
        for i in range(0, n - len_window + 1, stride)
    ]
    ...
    for window in windows:
        ...
        word = "".join(
            [alphabet[symbol] for symbol in symbols]
        )  # Mot SAX pour cette fenêtre
...
def sax_for_set(X, len_window, dim_window, alphabet_size, stride):
    ...
    sax_results = []
    for signal in X:
        sax_result, windows, breakpoints = sax(
            signal, len_window, dim_window, alphabet_size, stride
        )
...
def plot_sax_with_breakpoints(
    signal, sax_words, windows, len_window, dim_window, stride, breakpoints, alphabet
):
    """
    ...
    plt.title("SAX representation of a signal")
    plt.xlabel("Time")
