...
def extract_subsequences(time_series, sub_len):
    ...
    return np.lib.stride_tricks.as_strided(
        time_series, shape=(n_subs, sub_len), strides=(strides, strides)
    )
...
def information_gain(cand, X, y, metric="eucl"):
    ...
    for threshold in all_thresholds:
        ...
        weighted_entropy = (len(group1) / len(y)) * entropy_group1 + (
            len(group2) / len(y)
        ) * entropy_group2
        ...
        if information_gain_value > best_information_gain or (
            information_gain_value == best_information_gain
            and separation_gap > best_separation_gap
        ):
...
def eval_candidates(top_k_TS, X, y, metric="eucl"):
    ...
    for cand in top_k_TS:
        information_gain_value, separation_gap, best_threshold = information_gain(
            cand, X, y, metric=metric
        )
        if information_gain_value > max_gain or (information_gain_value == max_gain) & (
            separation_gap > min_gap
        ):
