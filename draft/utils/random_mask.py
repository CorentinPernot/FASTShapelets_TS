...
def compute_collision_matrix(nb_iterations, dict_sax):
    ...
    mot_to_index = {
        mot: idx for idx, mot in enumerate(mots)
    }  # pour accéder facilement aux indices des mots
    ...
    for iteration in range(nb_iterations):
        ...
        for idx_objet, (key, words) in enumerate(
            dict_sax.items()
        ):  # on parcourt les objets et les mots de chaque objet
            already_updated = []
            for word in words:  # pour chaque mot
                ...
                for (
                    key,
                    value_list,
                ) in new_mot_index.items():  # on trouve les autres mots masqués associés et leurs indices (correspondant aux mots non masqués)
                    if idx_word in value_list:
                        found_list = value_list
                        break
                for idx_word in found_list:
                    if (
                        idx_word not in already_updated
                    ):  # si deja updated alors on n'update pas
...
def compute_distinguish_power(nb_iterations, collision_matrix, y):
    ...
    distinguish_power = np.abs(close_to_ref[:, 0] - far_from_ref[:, 0]) + np.abs(
        close_to_ref[:, 1] - far_from_ref[:, 1]
    )
    ...
    for i in range(len(filtered_sax_results)):
        for j in range(len(filtered_sax_results[i])):
            if filtered_sax_results[i][j] not in sax_to_ts.keys():
                sax_to_ts[filtered_sax_results[i][j]] = [
                    filtered_all_subsequences[i][j]
                ]
            else:
                sax_to_ts[filtered_sax_results[i][j]].append(
                    filtered_all_subsequences[i][j]
                )
    return sax_to_ts
